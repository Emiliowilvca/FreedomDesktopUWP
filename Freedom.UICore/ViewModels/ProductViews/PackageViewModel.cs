...
namespace Freedom.UICore.ViewModels.ProductViews
{
    ...
    {
        private readonly string _controller = "/Packages";
        private readonly string _postMethod = "";
        private readonly string _putMethod = "";
        private readonly IPackageService _packageService;
        private readonly IValidator<IPackage> _validator;

        public PackageViewModel(IPackageService packageService,
                                IValidator<IPackage> validator)
        {
            PageTitle = new PageTitle(Lang.Package, MaterialDesignIcons.BottleSodaClassic);
            ...
            Package = new PackageBind();
        }

        #region Properties

        private PackageBind _package;

        public PackageBind Package { get => _package; set => SetProperty(ref _package, value); }
        ...
        {
            EnableControl = false;
            Package.CompanyId = _variableService.CompanyRTO.Id;
            _focusService.SetTextboxFocus("txtName");
        }

        private void EditCommandExecute()
        {
            bool resp = WeakReferenceMessenger.Default.IsRegistered<VMSendMessage<PackageINFO>, string>(this, _sendMessageToken);
            if (resp)
            {
                WeakReferenceMessenger.Default.Unregister<VMSendMessage<PackageINFO>, string>(this, _sendMessageToken);
            }
            WeakReferenceMessenger.Default.Register<VMSendMessage<PackageINFO>, string>(this, _sendMessageToken,
                (r, m) =>
                {
            ...
            _navigationService.NavigateTo<PackageSearchPage>(new NavigationParameter(1, _sendMessageToken.ToString()));
        }
        ...
        private async void SaveCommandExecute()
        {
            ...
            PackageDto packageDto = _mapper.Map<PackageDto>(Package);
            ...
            try
            {
                ...
                if (packageDto.Id == 0)
                {
                    XResponse resp = await _packageService.PostAsync(_prefix, _version, _controller, _postMethod, packageDto,
                ...
                {
                    XResponse resp = await _packageService.PutAsync(_prefix, _version, _controller, _putMethod, packageDto,
                                                    _cancelTokenSource.Token, _variableService.Token);
                    if (!resp.Success)
                    {
                        IsBusy = false;
                ...
                _reloadService.NotifyChanges(typeof(IPackage));
            }
        ...
        {
            ValidationResult result = _validator.Validate(Package);
            if (!result.IsValid)
            {
